...
def parse_cdp_neighbors(command_output):
    ...
    command_list = [elem.strip() for elem in command_list]
    res = {}
    for command in command_list:
            main = []
            second = []
            if command.find('>') != -1:
                main_machine = command[:command.find('>')]
            elif command.find('Eth') != -1:
                second_machine, main_eth, main_inter, *other, second_eth, second_inter = command.split()
                ...
                res[main_tuple] = second_tuple
    return res


if __name__ == "__main__":
    with open("11_modules\sh_cdp_n_r3.txt") as f:
